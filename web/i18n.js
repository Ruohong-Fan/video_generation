...
(function () {
  ...
  const ZH = {
    ...
    'Duration (s)': '时长 (秒)',
    'Count': '数量',
    'how many to generate per run': '每次运行生成多少个',
    'Loop the whole workflow N times — each pass appends a fresh result to every node':
      '循环运行整个工作流 N 次 — 每次都会为每个节点追加一个新结果',
    'Aspect ratio': '宽高比',
    'Resolution': '分辨率',
    'Model': '模型',
    'Run this node': '运行此节点',
  ...
  const PATTERNS_ZH = [
    ...
    [/^Pass\s+(\d+)\/(\d+)…$/, '第 $1/$2 次…'],
    [/^All\s+(\d+)\s+passes done$/, '$1 次循环全部完成'],
    [/^(text|image|video|audio|edit)\s+done\s+\(×(\d+)\)$/i, '$1 已完成 (×$2)'],
